...
import { controllerFileName, separator, prefix } from "./const/router.const.js";
...
async function loadRoutes() {
  async function recursiveRead(dir) {
    ...
    const files = entries.filter(
      (dirent) => dirent.isFile() && dirent.name === controllerFileName,
    );
    ...
    for (const file of files) {
      ...
      const parsedPath = relativePath
        .substring(prefix.length)
        .replaceAll(path.sep, separator);

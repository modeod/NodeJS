...
import {
  ...
  successHTMLResponse,
  successResponse,
} from "./helpers/responceHandler.js";
    ...
    .createServer(async (req, res) => {
      ...
      if (handlerType === "POST" || handlerType === "PUT") {
        if (!req.headers["content-type"])
          return errorResponse(res, `Couldn't process 'Content-Type' header`);
        const contentType = req.headers["content-type"].split(";")[0];
        const bodyParser = parseOptions[contentType];
        if (!bodyParser)
          return errorResponse(
            res,
            `Make sure that content type is either text, json or urlencoded`,
          );
      ...
      try {
        ...
        if (req.url.startsWith("/html") || req.url === "/") {
